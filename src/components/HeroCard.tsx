...
export function HeroCard() {
  ...
  return (
    <TiltCard className="rounded-2xl border border-black/[0.08] dark:border-white/[0.08] bg-white/60 dark:bg-white/[0.04] backdrop-blur-sm p-8 flex flex-col justify-between min-h-[280px] h-full overflow-hidden relative">
      {/* Accent blob */}
      ...
      <div className="flex items-start justify-between gap-6">
        <div className="flex flex-col gap-3 flex-1">
          ...
          <h1 className="text-3xl font-bold text-[#111] dark:text-white leading-tight">
            {t("title", { name: owner.name })}
          </h1>
          <p className="text-sm text-[#111]/60 dark:text-white/60 font-medium">
            {t("subtitle")}
          </p>
        ...
        <div className="shrink-0">
          <div className="w-20 h-20 rounded-2xl overflow-hidden border-2 border-white shadow-md">
            <Image
              src="https://picsum.photos/seed/minh-tam-biz/200/200"
              alt={owner.name}
              width={80}
              height={80}
